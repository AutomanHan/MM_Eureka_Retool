        ...
        for query, prompt, answer in zip(queries, prompts, labels):
            try:
                ...
                if response == "":
                    ...
                    run_code_acc = 0.2 if accuracy_reward > 0.0 else 0.0

                    reward_tmp = format_reward + format_code_reward + accuracy_reward #+ run_code_acc
                    rewards.append(reward_tmp)
                    accuracy_rewards.append(accuracy_reward)
                    format_rewards.append(format_reward)
                    format_code_rewards.append(format_code_reward)
                    run_code_rewards.append(run_code_acc)
                    f.write(f"===============================================================\n")
                    f.write("Query: " + query1 + "\n")
                    f.write("Response: " + response + "\n")
                    f.write("Answer: " + answer + "\n")
                    f.write(f"Accuracy Reward: {accuracy_reward}\tFormat Reward: {format_reward} \t Formatcode Reward: {format_code_reward} \t Runcode Reward: {run_code_acc} \n\n\n\n")
                    f.write(f"===============================================================\n")
            except:
                f.write("Error: " + query + "\n")
                rewards.append(0.0)
